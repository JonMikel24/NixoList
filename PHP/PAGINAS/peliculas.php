...
<?php } elseif ($seccion == 'recomendadas') {
    ...
    echo "<h2>Películas Más Populares</h2>";
    echo "<div class='grid-galeria'>";
    $paginaPopulares = callAPI("https://api.themoviedb.org/3/movie/popular?api_key=".$tmdb_key);
    
    foreach($paginaPopulares["results"] as $movie){
        $id = $movie["id"];
        $img = "https://image.tmdb.org/t/p/w500" . $movie["poster_path"];
        $title = $movie["title"];
        echo "<div class='card'><a href='media.php?id=$id&type=movie'><img src='$img'></a><p>$title</p></div>";
    ...
    echo "<h2>Top Películas de Todos los Tiempos</h2>";
    echo "<div class='grid-galeria'>";
    $paginaTop = callAPI("https://api.themoviedb.org/3/movie/top_rated?api_key=".$tmdb_key);
    
    foreach($paginaTop["results"] as $movie){
        $id = $movie["id"];
        $img = "https://image.tmdb.org/t/p/w500" . $movie["poster_path"];
        $title = $movie["title"];
        echo "<div class='card'><a href='media.php?id=$id&type=movie'><img src='$img'></a><p>$title</p></div>";
